...
int main()
{
  ...
  // Example with HYU-MM URDF.

  // ---------------------------------------------------------------------
  // Create a model based on a URDF file
  // ---------------------------------------------------------------------
  std::string urdf_filename = ws_path+"/urdf2model/models/hyu_mm/hyu_mm.urdf";
  ...
  std::vector<std::string> required_Frames = {"lin_x_joint", "lin_y_joint", "rot_z_joint", "Arm_Joint_1", "Arm_Joint_2", "Arm_Joint_3", "Arm_Joint_4", "Arm_Joint_5","Arm_Joint_6", "endeffector"};
  ...
  mecali::generate_code(fd, "hyu_mm_fd", codegen_options);
  mecali::generate_code(id, "hyu_mm_id", codegen_options);
  mecali::generate_code(G, "hyu_mm_G", codegen_options);
  //mecali::generate_code(fk_ee_pos, "mmo500_ppr_fk_ee_pos", codegen_options);
   mecali::generate_code(fkrot_ee, "hyu_mm_fkrot_ee", codegen_options);
  mecali::generate_code(fk_ee, "hyu_mm_fk_ee", codegen_options);
  mecali::generate_code(fk, "hyu_mm_fk", codegen_options);
  mecali::generate_code(J_fd, "hyu_mm_J_fd", codegen_options);
  mecali::generate_code(J_id, "hyu_mm_J_id", codegen_options);

  robot_model.generate_json("hyu_mm.json");
