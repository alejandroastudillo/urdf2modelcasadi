...
int main()
{
    ...
    // ---------------------------------------------------------------------
      std::string urdf_filename = "../urdf2model/models/rubion/online_rubion.urdf";
    ...
    // Set function for forward kinematics
      ...
      casadi::Function fk_ee    = robot_model.forward_kinematics("transformation", "link6");
      ...
      robot_model.generate_json("rubion.json");
    ...
    // Code-generate or save a function
      ...
      mecali::generate_code(fk_ee, "rubion_fk_ee", codegen_options);
      mecali::generate_code(fk, "rubion_fk", codegen_options);
