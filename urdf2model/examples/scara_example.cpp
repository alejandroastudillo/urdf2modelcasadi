...
int main(int argc, char ** argv)
{
  ...
  // ---------------------------------------------------------------------
  // std::string urdf_filename = "../urdf2model/models/epson_modified_files/Epson001.urdf";
  // Example with robot urdf passed as argument, or Kinova Gen3 URDF by default.
  std::string robot = (argc<=1) ? "Epson001" : argv[1];
  // std::string urdf_filename = "../urdf2model/models/epson_modified_files/" + robot + ".urdf";
  std::string urdf_filename = "../urdf2model/models/planar/" + robot + ".urdf";
  ...
  mecali::generate_code(fwd_dynamics, robot+"_fd", codegen_options);
  mecali::generate_code(inv_dynamics, robot+"_id", codegen_options);

  mecali::generate_code(fk_T, robot+"_fk", codegen_options);

  // mecali::generate_code(regressor, "full_scara_regressor_mod2", codegen_options);

  mecali::generate_code(mass_inverse, robot+"_mass_inverse", codegen_options);
  mecali::generate_code(coriolis, robot+"_coriolis", codegen_options);
  mecali::generate_code(gravity, robot+"_gravity", codegen_options);
