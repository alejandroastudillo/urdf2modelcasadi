...
int main()
{
  ...
  // ---------------------------------------------------------------------
  std::string urdf_filename = ws_path+"/urdf2model/models/sftlink/sim_assm.urdf";
  ...
  std::vector<std::string> required_Frames = {"joint1", "joint2", "joint3", "joint4", "joint5", "endeffector"};
  ...
  mecali::generate_code(fd, "sftlink_fd", codegen_options);
  mecali::generate_code(id, "sftlink_id", codegen_options);
  mecali::generate_code(G, "sftlink_G", codegen_options);
  //mecali::generate_code(fk_ee_pos, "sftmp_fk_ee_pos", codegen_options);
   mecali::generate_code(fkrot_ee, "sftlink_fkrot_ee", codegen_options);
  mecali::generate_code(fk_ee, "sftlink_fk_ee", codegen_options);
  mecali::generate_code(fk, "sftlink_fk", codegen_options);
  mecali::generate_code(J_fd, "sftlink_J_fd", codegen_options);
  mecali::generate_code(J_id, "sftlink_J_id", codegen_options);

  robot_model.generate_json("sftlink.json");
