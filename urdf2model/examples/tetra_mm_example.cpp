...
int main()
{
  ...
  // ---------------------------------------------------------------------
  std::string urdf_filename = ws_path+"/urdf2model/models/tetra/open_manipulator_p_robot.urdf";
  ...
  std::vector<std::string> required_Frames = {"lin_x_joint", "lin_y_joint", "rot_z_joint", "joint1", "joint2", "joint3", "joint4", "joint5","joint6", "endeffector", "base_env"};
  ...
  mecali::generate_code(fd, "tetra_mm_fd", codegen_options);
  mecali::generate_code(id, "tetra_mm_id", codegen_options);
  mecali::generate_code(G, "tetra_mm_G", codegen_options);
  //mecali::generate_code(fk_ee_pos, "mmo500_ppr_fk_ee_pos", codegen_options);
   mecali::generate_code(fkrot_ee, "tetra_mm_fkrot_ee", codegen_options);
  mecali::generate_code(fk_ee, "tetra_mm_fk_ee", codegen_options);
  mecali::generate_code(fk, "tetra_mm_fk", codegen_options);
  mecali::generate_code(J_fd, "tetra_mm_J_fd", codegen_options);
  mecali::generate_code(J_id, "tetra_mm_J_id", codegen_options);

  robot_model.generate_json("tetra_mm.json");
